...
Sub Process_Globals
	Public URL As String : URL = "http://www.iranapp.org/bahonar/index.php/server/"
...
Sub WriteCounterReadBook(BookName As String)
	...
	If File.Exists(File.DirInternal,"visit_book") Then
		m = File.ReadMap(File.DirInternal,"visit_book")
	End If
	...
	File.WriteMap(File.DirInternal,"visit_book",m)
	
End Sub

Sub GetTopReadBook As String()
...
Sub SortMapKeys (m As Map, SortAsc As Boolean)
   ...
   For i = 0 To m.Size - 1
      Private val As String = m.GetValueAt(i)
      KeysList.Add(val)
   Next
   ...
   For x=0 To KeysList.Size - 1
       Private val As String = KeysList.Get(x)
       Private key As String = m.Get(val)
       m2.Put(key, val)
   Next
  m.Clear
...
Sub Setting
	...
	Dim pre As AHPreferenceScreen
	pre.Initialize("تنظیمات","تنظیم کتاب")
	
	Dim prem As AHPreferenceCategory
	prem.Initialize("بخش کتاب")
 
	prem.AddCheckBox("state","تنظیم وضعیت خواندن کتاب به شب","وضعیت روز","وضعیت شب",False,"")
	prem.AddList("fontfamily","قلم کتاب","انتخاب قلم متن کتاب","byekan","",Array As String("tahoma","byekan","قرآنی","bnazanin"))
	...
	For i = 13 To 50
		ls.Add(i)	
	Next
	prem.AddList("fontsize","اندازه قلم کتاب","انتخاب اندازه قلم متن کتاب","14","",ls)

	pre.AddPreferenceCategory(prem)
	StartActivity(pre.CreateIntent)
